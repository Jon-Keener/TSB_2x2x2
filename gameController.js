...
// console.log( 'startGame()' );
    ...
    for (let i = 0; i < p1rack.length; i++) {
        ...
        let tileandtier = p1rack[i].replace(/\d/g, '') + '0';
        p1rack[i].piece = getPieceImageSource( tileandtier ); // tileandtier yields the piece img element.
        ...
        // calculate tileandtier
        tileandtier = p2rack[i].replace(/\d/g, '') + '0'; // tileandtier yields the piece img element. // Tile and tier
    ...
    updateBoardsFromRack1();
    updateBoardsFromRack2();
...
function updateBoardsFromRack1() {
...
function updateBoardsFromRack2() {
...
// console.log( 'onPerimeter( ', new_ary, '):', onPerimeter( new_ary ) ); 
                    ...
                    if ( periflowerUp === 0 ) {
                        console.error( 'checkWords() returns 200 for an UP word.' );
...
// console.log( 'onPerimeter( ', new_ary, '):', onPerimeter( new_ary ) ); 
                    ...
                    if ( periflowerDn === 0 ) {
                        console.error( 'checkWords() returns 201 for a DW word.' );
                        console.error( 'The first word must be on a perimeter flower.  Undo the last turn.' );
                        return 201;
                    }
                    break;
...
// console.log( 'onPerimeter( ', new_ary, '):', onPerimeter( new_ary ) ); 
                    ...
                    if ( periflowerDo === 0 ) {
                        console.error( 'checkWords() returns 202 for a DO word.' );
                        console.error( 'The first word must be on a perimeter flower.  Undo the last turn.' );
                        return 202;
                    }
                    break;
        ...
        // else { // end of first word check, every other word needs to be on a flower OR on or next to a previous word played
        // }
